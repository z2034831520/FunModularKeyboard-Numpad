...
void DisplayTask::run() {
    ...
    while(1) {
        ...
        // // 非阻塞方式检查队列
        // if (xQueueReceive(knob_state_queue_, &state, 0) == pdPASS) {
        //     // 有数据，处理状态
        //     char buf_[256] = {0};
        //     snprintf(buf_, sizeof(buf_), "state.current_position=%d,sub_position_unit=%f",state.current_position, state.sub_position_unit);
        //     log(buf_);
        //  } 
        //else {
        //     // 无数据，处理其他任务
