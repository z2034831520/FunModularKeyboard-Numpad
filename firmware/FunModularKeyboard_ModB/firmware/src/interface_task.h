...
class InterfaceTask : public Task<InterfaceTask>, public Logger {
    ...
    private:
        ...
        int32_t selected_led_color_index_ = 0;
        bool led_color_dirty_ = false;
        uint32_t last_led_color_change_ms_ = 0;
        uint32_t last_knob_state_ms_ = 0;
        ...
        int switchLEDs(int index);
        void updateHardware();
        void publishState();
        void persistLedColorSelection(bool force);
        void applyConfig(PB_SmartKnobConfig& config, bool from_remote);

        I2CSlaveDevice i2cSlave_;///
};
